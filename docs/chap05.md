...
`#define WEBSERVER`  
If you activate this definement, you can use BSB-LAN as web server for static content. All files stored on SD card. Files can be placed in different directories.
Supported files types: html, htm, css, js, xml, txt, jpg, gif, svg, png, ico, gz.
Web server support static compression. It always trying deliver gzipped content (Ex.: /d3d.js.gz for /d3d.js URL) when it possible (if client browser accepting gzip).
Web server support these headers: ETag, Last-Modified, Content-Length, Cache-Control.
Note:
If web server found index.html when user request / URL then it send index.html instead internal web server.
